...
const FormFilter = (props:any) => {
  ...
  const setCardsDisplayed:(c: ICard[]) => void = contextAppValue.setCardsDisplayed;
      ...
      <Formik
        ...
        onSubmit={(values) => {
          ...
          for (let i = 0; i < cardsDisplayed.length; i++) {
            let nameMatch = (new RegExp(patientName)).test(cardsDisplayed[i].patient_name);

            if (arrhythmia === "all") {
              cardsDisplayed[i].display = nameMatch;
            } else {
              let test1 = patientName === "" ? true : nameMatch;
              let test2 = cardsDisplayed[i].arrhythmias.find(name => name === arrhythmia)
                ? true : false;
            ...
            <Row>
              ...
              <Col>
                <InputGroup className="mb-3">
                  ...
                  <Button variant="primary" id="button-addon2" type="submit" onClick={(e?: any):void => { handleSubmit(e) }}>
                    Filtrer
                  </Button>
                </InputGroup>
              </Col>
